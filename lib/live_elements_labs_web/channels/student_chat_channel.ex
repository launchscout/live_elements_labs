defmodule LiveElementsLabsWeb.StudentChatChannel do
  use LiveState.Channel, web_module: LiveElementsLabsWeb, json_patch: true
  alias Phoenix.PubSub
  alias Nostrum.Api

  def guild_id, do: Application.get_env(:live_elements_labs, :guild_id)

  def init(_socket, _payload, _channel) do
    {:ok, %{messages: [], conversation_started: false}}
  end

  def handle_event("start_chat", %{"name" => name}, state) do
    {:ok, channel} = Api.create_guild_channel(guild_id(), name: "Student #{name}")
    PubSub.subscribe(LiveElementsLabs.PubSub, "messages:#{channel.id}")
